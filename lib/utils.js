  // var noise5 = new Audio();
  // //fxJump.addEventListener("canplaythrough", playSoundHandler, false);
  // noise5.src = `noise5.wav`;
  // noise5.volume = 0.5;

var passwordsolved = false;
...
function RayCast(x, y, dir, platforms, maxI, render, cam)
{
  ...
  while(i < maxI && collision != true && rx - cam.x < 10000 && rx - cam.x > -10000 && ry - cam.y < 10000 && ry - cam.y > -10000)
  {
    ...
    // noise5.loop = false; noise5.pause(); noise5.currentTime = 0; noise5.play();
    ...
    if(dist.minD < 1.5)
    {
      ...
      collision = true;
      if(platforms.platforms[dist.minI].type >= 2 && platforms.platforms[dist.minI].type <= 3) {
		...
		segments = RandomInt(1, 2);
		console.log(segments);
        var cols = RandomInt(1, 2);
        var rows = RandomInt(1, 2);
      ...
      else if(platforms.platforms[dist.minI].type == 4 || platforms.platforms[dist.minI].type == 5)
      {
        platforms.platforms.splice(dist.minI, 1);
      }
      else if (platforms.platforms[dist.minI].type == -1) {
        // destroyBlock[4]--;
        // if (destroyBlock[4] < 1) {        
          for (var i = 0; i < index.length; i++) {
            if(platforms.platforms[dist.minI] == index[i]) {
              boom(destroyblocks[i][0], destroyblocks[i][1], destroyblocks[i][3], dist.minI);
            }
          }
          //platforms.platforms.splice(dist.minI, 1);
        // }
...
// To make it split 3 times
    ...
    //       for(var x = plat.x1; x <= plat.x2 - w / cols; x += w / cols)
    //       {
    //         platforms. (x, y, x + w / cols, y + h / rows, plat.type + 1);
    //       }
    //     }
    //   }
