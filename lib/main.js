...
class Player
{
  ...
  Death()
  {
    // (Math.abs(this.x) > (this.MaxPos * 50) || Math.abs(this.y) > (this.MaxPos * 50)
    if((this.health <= 0) && death == false)
    {
      this.w = 0;
      this.h = 0;
      this.xv = 0;
      ...
      for (var i = 100 - 1; i >= 0; i--) {
        particles.Add(this.x, this.y, RandomInt(-10, 10), RandomInt(-10, 10), RandomInt(2, 10), {r: 255, g: 255, b: 255, a: 1}, "blood");
      }
      death = true;
      setTimeout(function(){deathScreen();}, 500);
      // location.reload();
    }
  }
...
class Platform
{
  ...
  Draw(render, cam)
  {
    if(this.x1 - cam.x <= render.Width && this.x2 - cam.x >= 0 && this.y1 - cam.y <= render.Height && this.y2 - cam.y >= 0)
    {
        ...
        {
          render.SetFill(212, 175, 55, 1);
          render.SetStroke(255, 215, 0, 1);
          render.DrawFillRectNoStroke(this.x1 - cam.x, this.y1 - cam.y, this.x2 - cam.x, this.y2 - cam.y);
        }
        else if(this.type == -1)
        {
          render.SetFill(0, 255, 0, 1);
...
class PlatformArray
{
  ...
  Collision(x, y, w, h, render, cam = {x: 0, y: 0}, hitbox = false)
  {
    for(var i in this.platforms)
    {
      ...
      {
        if(platforms.platforms[i].type == 7) {
          player.health = 0;
        }
        return true;
      }
...
function boom(x1, y1, blocksdestroyed, destroyblock) {
  var scaling = 166 * blocksdestroyed;
  platforms.platforms[destroyblock].x1 -= scaling;
  platforms.platforms[destroyblock].x2 += scaling;
  platforms.platforms[destroyblock].y1 -= scaling;
  platforms.platforms[destroyblock].y2 += scaling;
  if(player.x > platforms.platforms[destroyblock].x1 && player.x < platforms.platforms[destroyblock].x2 && player.y > platforms.platforms[destroyblock].y1 && player.y < platforms.platforms[destroyblock].y2) {
    player.health -= 50;
  }
    var distx = Math.abs(platforms.platforms[destroyblock].x1 - platforms.platforms[destroyblock].x2);
    var startx = platforms.platforms[destroyblock].x1;
    var disty = Math.abs(platforms.platforms[destroyblock].y1 - platforms.platforms[destroyblock].y2);
    var starty = platforms.platforms[destroyblock].y1;

    for (var i = platforms.platforms.length - 1; i >= 0; i--) {
        if(platforms.platforms[i].x1 > platforms.platforms[destroyblock].x1 && platforms.platforms[i].x2 < platforms.platforms[destroyblock].x2 && platforms.platforms[i].y1 > platforms.platforms[destroyblock].y1 && platforms.platforms[i].y2 < platforms.platforms[destroyblock].y2)
        {
            if (platforms.platforms[i].type == 2 || platforms.platforms[i].type == 3 || platforms.platforms[i].type == 4 || platforms.platforms[i].type == 7) {
                platforms.platforms.splice(i, 1);
            }
            else if(platforms.platforms[i].type == -1)
            {
              if (deathboxesblownup.length == 0) {
                deathboxesblownup.push(platforms.platforms[i]);
              }
              else {
              var bool = false;
                for (var j = 0; j < deathboxesblownup.length; j++) {
                  if (deathboxesblownup[j] == platforms.platforms[i])
                  {
                    bool = true;
                  }
                }
    ...
    for (var j = 0; j < deathboxesblownup.length; j++) {
      if (deathboxesblownup[j] == platforms.platforms[destroyblock])
      {
        deathboxesblownup.splice(j, 1);
      }
    }
    platforms.platforms.splice(destroyblock, 1);

      for (var i = 0; i < distx; i+=75) {
        for (var j = 0; j < disty; j+=75) {
          particles.Add(startx + i, starty + j, RandomInt(-10, 10), RandomInt(-30, 10), RandomInt(2, 10), {r: 0, g: 255, b: 0, a: 1}, "blood");
        }
      }
    var indexnum;
    if (deathboxesblownup.length > 0) {
      for (var j = 0; j < deathboxesblownup.length; j++) {
        for (var k = 0; k < index.length; k++) {
          if (deathboxesblownup[j] == index[k])
          {
            indexnum = k;
          }
        }
      }
      for (var j = 0; j < platforms.platforms.length; j++) {
          if (platforms.platforms[j] == index[indexnum])
          {
            countdownDS = 3;
            boxtoblow = [destroyblocks[indexnum][0], destroyblocks[indexnum][1], destroyblocks[indexnum][3], j];
          }
        }
      }
}
